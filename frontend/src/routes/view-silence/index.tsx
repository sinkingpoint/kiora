...
const silenceView = (silence: Silence) => {
    ...
    const startString = hasntStartedYet ? `In ${formatDuration(secondsSinceStart)}` : `${formatDuration(secondsSinceStart)} Ago`;
    const endString = hasEnded ? `${formatDuration(secondsTillEnd)} Ago` : `In ${formatDuration(secondsTillEnd)}`;
    ...
    return <div>
        ...
        <div>
            <h2>Started at</h2>
            <p>{formatDate(startTime)} ({startString})</p>
        </div>

        <div>
            <h2>Ends at</h2>
            <p>{formatDate(endTime)} ({endString})</p>
        </div>
    </div>
}
