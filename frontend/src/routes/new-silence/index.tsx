...
const LabelMatcher = (matcher: string, onDelete: () => void) => {
    ...
    return <span>
        ...
        <button type="button" onClick={onDelete} class={style['delete-label-button']}>x</button>
    </span>;
}
...
const NewSilence = () => {
    ...
    const end = endDate !== null ? <span>Ends at {endDate.toLocaleString([], {day: 'numeric', month: 'short', year:'numeric', hour: '2-digit', minute:'2-digit'})}</span> : <span>Invalid duration</span>;
            ...
            <form>
                <table>
                    <tr>
                        ...
                        <td><input type="text" title="Duration in Go format, e.g. 1h" pattern="[0-9]+[mhdw]" value={duration} onInput={handleSetDuration} /></td>
                        <td>{end}</td>
                    </tr>
